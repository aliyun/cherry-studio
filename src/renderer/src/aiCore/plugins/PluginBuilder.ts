...
export interface BuildPluginsContext {
  provider: Provider
  model: Model
  config: AiSdkMiddlewareConfig & { assistant: Assistant }
}
...
export function buildPlugins(
  { provider, model, config }: BuildPluginsContext,
  ...
  if (config.isSupportedToolUse || config.isPromptToolUse) {
    plugins.push(searchOrchestrationPlugin(config.assistant, traceContext))
  }
