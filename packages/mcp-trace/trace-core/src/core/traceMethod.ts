...
export function Traced<This, Args extends any[], Return>(
  ...
  return (
    ...
    const traceName =
      options?.traceName || defaultConfig.defaultTracerName || "default";
...
// typescrit 5.0+
export function TracedAsync<This, Args extends any[], Return>(
  options?: SpanDecoratorOptions
) {
  return (
...
export function TraceMethod(
  ...
  return function (
    ...
    const traceName =
      tracerName || defaultConfig.defaultTracerName || "default";
    const tracer = trace.getTracer(traceName);
