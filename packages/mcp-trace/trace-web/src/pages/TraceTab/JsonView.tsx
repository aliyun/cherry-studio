    ...
    const renderString = (value: string) => {
      ...
      if (!isTooLong) {
        return (
          <div className="flex mr-1 rounded hover:bg-gray-800/20">
            {name && (
              <span className="mr-1 text-gray-600 dark:text-gray-400 break-all whitespace-pre-wrap overflow-wrap break-word">
                {name}:
              </span>
            )}
            <pre
              className={clsx(
                ...
                'break-all whitespace-pre-wrap overflow-wrap break-word',
              )}
            >
              {value}
            </pre>
      ...
      return (
        <div className="flex mr-1 rounded group hover:bg-gray-800/20">
          {name && (
            <span className="mr-1 text-gray-600 dark:text-gray-400 dark:group-hover:text-gray-100 group-hover:text-gray-400 break-all whitespace-pre-wrap overflow-wrap break-word">
              {name}:
            </span>
          )}
          <pre
            className={clsx(
              isError ? typeStyleMap.error : typeStyleMap.string,
              'cursor-pointer break-all whitespace-pre-wrap overflow-wrap break-word',
            )}
            onClick={() => setIsExpanded(!isExpanded)}
